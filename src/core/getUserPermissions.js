...
async function getUserPermissions(bearertoken, guildId) {
    try { // I really don't think the config needs to exist here
        let permissions = [] // Permissions should be definitely an array
        let permissionsint // Initialise permissions integer
        let guilds = await getGuilds(bearertoken) // Let's get our guilds
        for (i in guilds) { // For each returned guild, get their permissions integer
            if (guilds[i].id == guildId) { // The guild ID here matches so we can manipulate the permissions integer, yay!
                permissionsint = guilds[i]["permissions"]
                if (permissionsint) {
                ...
                break; // Break the loop since we don't need to continue checking for guilds' permissions, we've reached our target guild
            }
        }
    ...
    } catch (err) { // If there's an error, void it and return a blank array.
        void err
        return []
    }
}
