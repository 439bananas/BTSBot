...
function info(message) { // Log depending on function called
    getlang(true).then(lang => {
        if (path.basename(process.argv[1]) == "index.cjs") { // Tell translate() where the i18n file is depending on if we're in the server or not
...
function warn(message) {
    getlang(true).then(lang => {
        if (path.basename(process.argv[1]) == "index.cjs") {
            translateMode = "express-engine-jsx"
        } else translateMode = undefined
