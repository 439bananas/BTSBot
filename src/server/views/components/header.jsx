...
if (conf && isMod) { // Check if user is classed as a mod in the configured guild and add options based on that. If no conf or user not signed in, isMod will always return false
    modDropdownOptions = <div>
        <li><a className="dropdown-item" href="/helpdesk">{translate(lang, "page_globalhelpdesk")}</a></li>
        <li><a className="dropdown-item" href="/all-servers">{translate(lang, "page_globalallservers")}</a></li>
        <li><a className="dropdown-item" href="/user-manager">{translate(lang, "page_globalusermanager")}</a></li>
    </div>
} else {
    modDropdownOptions = " "
}
...
if (!conf) { // Determine what's shown in the area of the sign-in button
    ...
    signinbutton = <button type="button" id="dashboard-button" className="button-1x2ahC button-38aScr lookFilled-1Gx00P colorGreen-29iAKY sizeSmall-2cSMqn grow-q77ONN" onClick={"window.location.href = " + uniconf.metadomain}>
        <div className="contents-18-Yxp">{translate(lang, "page_noconfdashboard")}</div>
    </button>
    ...
    signinbutton = <button type="button" id="SignInButton" className="button-1x2ahC button-38aScr lookFilled-1Gx00P colorGreen-29iAKY sizeSmall-2cSMqn grow-q77ONN">
            <div className="contents-18-Yxp">{translate(lang, "page_globalsigninwithdiscord")}</div>
        </button>
    ...
    signinbutton = <div className="dropdown" id="logged-in-dropdown">
        ...
        <ul className="dropdown-menu" aria-labelledby="userDropdown">
            <li><a className="dropdown-item" href="/servers">{translate(lang, "page_globalservers")}</a></li>
            {modDropdownOptions}
            <li><a className="dropdown-item" href="/settings">{translate(lang, "page_globalaccountsettings")}</a></li>
            <li><a className="dropdown-item" href="/logout">{translate(lang, "page_globalsignout")}</a></li>
        </ul>
    </div>
}
            ...
            <div className="collapse navbar-collapse" id="navbar">
                <ul className="navbar-nav">
                    <li className="nav-item">
                        <a href={"https://wiki." + uniconf.metadomain} className="nav-link">{translate(lang, 'page_globaldocumentation')}</a>
                    </li>
                    <li className="nav-item">
                        <a href={uniconf.discord} className="nav-link">{translate(lang, 'page_globaldiscord')}</a>
                    </li>
                    <li className="nav-item">
                        <a href="https://github.com/439bananas/BTSBot" className="nav-link">{translate(lang, "page_globalgithub")}</a>
                    </li>
                </ul>
